...
(() => {
  ...
  // Sample questions come from a curated pool of gold-set questions the
  // jury scored correct (static/sample_pool.js, generated by
  // build_sample_pool.py); each draw is stratified across categories by
  // static/sample_shuffle.js. Purely client-side -- no API call.
  const SAMPLE_POOL = window.AEROOPS_SAMPLE_POOL || [];
  ...
  function renderSamples() {
    if (!SAMPLE_POOL.length || !window.AeroOpsSamples) {
      els.sampleChips.parentElement.hidden = true;
      return;
    }
    const draw = window.AeroOpsSamples.drawSamples(SAMPLE_POOL, shownSampleIds);
    shownSampleIds = draw.map((q) => q.id);
    ...
    draw.forEach((item) => {
      ...
      tag.className = "sample-chip-category";
      ...
      // explicitly ask for. They still press Ask themselves.
      chip.addEventListener("click", () => {
        els.input.value = item.question;
        els.input.focus();
      });
  ...
  els.shuffleBtn.addEventListener("click", renderSamples);
  renderSamples();
