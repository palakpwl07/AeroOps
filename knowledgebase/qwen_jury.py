"""
AeroOps jury pass — Qwen3.8 Max (free tier via TokenRouter), retrieval-wired
to the local graph export.
...
NOTE: qwen/qwen3.8-max-free is TokenRouter's free tier — "compute capacity
is limited, service stability and concurrency are not guaranteed" per their
own docs. Retry logic below exists specifically because of that, this
wasn't a real risk with a paid endpoint. If a lot of items still fail after
retries, that's the free tier being genuinely overloaded, not a bug, rerun
later or fall back to a paid model.

Usage:
    export TOKENROUTER_API_KEY=...
    python qwen_jury_retrieval.py \
        --graph aeroops_knowledgegraph.md \
        --questions blind_questions.json \
        --out jury_qwen_answers.json
"""
...
def call_qwen(client, question, graph_facts, evidence, model="qwen/qwen3.8-max-free", max_retries=4):
...
def answer_one(client, item, nodes_by_id, chunks_by_id, relationships, mentioned_in, model, max_retries=4):
    ...
    try:
        parsed = call_qwen(client, question, graph_facts, evidence, model=model, max_retries=max_retries)
        status = "done"
    except Exception as e:
    ...
    return {
        "id": qid,
        "qwen_answer": parsed.get("answer"),
        "qwen_retrieved_chunk_ids": chunk_ids,
        "qwen_reasoning_chain": parsed.get("reasoning_chain", []),
    }, status, len(chunk_ids), len(graph_facts)
...
def main():
    ...
    ap.add_argument("--out", default="jury_qwen_answers.json")
    ap.add_argument("--model", default="qwen/qwen3.8-max-free")
    args = ap.parse_args()
    ...
    for i, item in enumerate(questions):
        result, status, n_chunks, n_facts = answer_one(
            client, item, nodes_by_id, chunks_by_id, relationships, mentioned_in, args.model
        )
        results.append(result)
        print(f"[{i+1}/{len(questions)}] {item['id']} {status} ({n_chunks} chunks, {n_facts} facts)")
        save(results, args.out)  # save after every item, not just at the end
        time.sleep(1.5)  # free tier — a bit more courtesy delay than a paid endpoint needs

    # --- Final retry sweep on anything still failed ---
    still_failed = [r["id"] for r in results if r["qwen_answer"] is None]
    if still_failed:
        print(f"\n{len(still_failed)} item(s) failed in the main pass: {still_failed}")
        print("Running a final retry sweep (longer patience, since these already failed once)...\n")
        by_id = {q["id"]: q for q in questions}
        results_by_id = {r["id"]: r for r in results}

        for qid in still_failed:
            print(f"Retrying {qid}...")
            time.sleep(3)  # give the free tier a bit more breathing room before hammering it again
            result, status, n_chunks, n_facts = answer_one(
                client, by_id[qid], nodes_by_id, chunks_by_id, relationships, mentioned_in,
                args.model, max_retries=6,  # more patience on the second try
            )
            results_by_id[qid] = result
            results = [results_by_id[q["id"]] for q in questions]  # rebuild in original order
            print(f"  {qid} {status} on retry sweep ({n_chunks} chunks, {n_facts} facts)")
            save(results, args.out)

    print(f"\nWrote {len(results)} items to {args.out}")

    final_failed = [r["id"] for r in results if r["qwen_answer"] is None]
    if final_failed:
        print(f"\n{len(final_failed)} item(s) still failed after the retry sweep: {final_failed}")
        print("The free tier is likely down or overloaded right now, not a script bug.")
        print("Rerun this script later — it will still re-attempt every item, including the")
        print("already-succeeded ones. If that's wasteful, ask for a --retry-failed-only mode")
        print("that reads the existing output file and only re-attempts the nulls.")
    else:
        print("\nAll items answered successfully.")
